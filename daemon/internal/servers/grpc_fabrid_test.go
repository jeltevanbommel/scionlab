...
func TestFetchFabridDetachedMaps(t *testing.T) {
	...
	mockClient.EXPECT().RemoteMaps(ctx, MatchesRemoteMapsRequest(&experimental.
		RemoteMapsRequest{IsdAs: uint64(xtest.MustParseIA("1-ff00:0:111")),
...
func TestFindDetachedHopsAllDetached(t *testing.T) {
	...
	expectedHops := []tempHopInfo{
		{IA: xtest.MustParseIA("1-ff00:0:110"), Digest: []byte("firstHopDigest"), fiIdx: 0, Ingress: 0, Egress: 1},
		{IA: xtest.MustParseIA("1-ff00:0:111"), Digest: []byte("middleHopDigest"), fiIdx: 1, Ingress: 2, Egress: 3},
		{IA: xtest.MustParseIA("1-ff00:0:112"), Digest: []byte("lastHopDigest"), fiIdx: 2, Ingress: 4, Egress: 0},
	}
