...
func New(intfIndex int, protocol Protocol, b bool) (RawSocketConn, error) {
	...
	if b {
		...
		flags = unix.SOCK_DGRAM | unix.SOCK_CLOEXEC | unix.SOCK_NONBLOCK

	}
	s, err := unix.Socket(unix.AF_PACKET, flags,
		int(proto))
	if err != nil {
...
func (m RawSocketConn) WriteBatch(messages []Mmsghdr, flags int) (int, error) {
	...
	if err := m.c.Write(func(fd uintptr) (done bool) {
		...
		n, _, errno = unix.Syscall6(unix.SYS_SENDMMSG, fd, uintptr(unsafe.Pointer(&messages[0])),
			uintptr(len(messages)), uintptr(flags|unix.MSG_WAITALL), 0, 0)
		if errno == 0 { // Reset the readerr, it can already be set due to previous retries.
			writeErr = nil
		} else {
			writeErr = errno
		}
		return !(writeErr != nil && errno.Temporary())
	}); err != nil {
		return 0, err
	}
	if writeErr != nil {
